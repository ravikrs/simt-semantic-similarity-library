 ...
 */
public class LuceneVectorReader extends IndexedDocumentsVectorReaderBase {
	...
	@Override
	public int getDocumentCount() throws SimilarityException {
		return getReader().numDocs();
	}

	@Override
	public int getDocumentFrequency(String aTerm) throws SimilarityException {
		try {
	...
	@Override
	public Set<Integer> getDocuments(String aTerm) throws SimilarityException {
		return getTermFrequenies(aTerm).keySet();
	}

	@Override
	public int getTermFrequency(int aDocId, String aTerm) throws SimilarityException {
		Integer count = getTermFrequenies(aTerm).get(aDocId);
		return (count == null) ? 0 : count;
	}

	@Override
	public int getIndexVocabularySize() throws SimilarityException {
		try {
	...
	@Override
	public int getDocumentLength(int aDocId) throws SimilarityException {
	...
	private Map<Integer, Integer> getDocumentVocabularySizes() throws SimilarityException {
		try {
	...
	private Map<Integer, Integer> getTermFrequenies(String aTerm) throws SimilarityException {
		...
		// Fetch the IDs from the index and at the same time cache the term
		// frequencies.
		...
		try {
			...
			// the semantics of lucene next() is different from
			// java.util.collections, initial
	...
	private IndexReader getReader() throws SimilarityException {
		try {
			if (reader == null) {
