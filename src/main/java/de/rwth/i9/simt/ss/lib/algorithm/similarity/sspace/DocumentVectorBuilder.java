 ...
 *
 * <p>
 * Documents will be tokenized using the current tokenizing method, and the
 * vector in the {@code SemanticSpace} corresponding to each word found in the
 * document will be combined together.
 * </p>
 *
 * <p>
...
public class DocumentVectorBuilder {
	 ...
	 * term vectors.
	 */
	public static final String USE_TERM_FREQUENCIES_PROPERTY = PROPERTY_PREFIX + ".usetf";
	 ...
	 *            A {@code Vector} which has been pre-allocated to store the
	 *            document's representation. This is pre-allocated so that users
	 *            of {@code
	 *                       DocumentVectorBuilder} can decide what type of
	 *            {@code Vector} should be used to represent a document.
	 *
	 * @return {@code documentVector} after it has been modified to represent
	 *         the terms in {@code document}.
	 */
	public DoubleVector buildVector(Collection<String> articleTokens, DoubleVector documentVector) {
		...
		for (String term : articleTokens) {
			...
			termCounts.put(term, (count == null || !useTermFreq) ? 1 : count.intValue() + 1);
		}
